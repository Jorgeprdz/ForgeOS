...
EVIDENCE_STATUS=PASS_IMPLEMENTATION
FINAL_SHA=BOUND_BY_FINAL_DOCUMENTATION_REVALIDATION
```
...
The browser evidence was generated by `.github/workflows/pipeline-aura-ux-reconciliation-001.yml` and remains attached to the workflow run that tested the implementation commit.
...
ARTIFACT_EXPIRES=2026-09-06T03:47:24Z
```

Artifact contents produced by the successful browser run:
...
The artifact name uses GitHub's pull-request merge test SHA, while the workflow metadata binds the run to branch head `57b60146a0d274acd67a857ec135887fb986989a`.
...
The next workflow run must revalidate this evidence/documentation commit. After that green run, the same PR body binds the final branch SHA and the reconciliation may be reported as PASS while remaining Draft.
