import { reconcileQuoteResult } from "./quotes-result-adapter-complete.js?v=manual-quotes-complete-001";
...
function ensureQuotesStyles() {
  ...
  stylePromise = new Promise((resolve, reject) => {
    ...
    stylesheet.href = new URL(
      "./quotes-module.css?v=manual-quotes-complete-001",
      import.meta.url,
    );
...
async function appendRuntimeScript(script, sourceUrl) {
  if (script.src.includes("forge-alive-mobile-nav-r16c5j")) return;

  const executable = document.createElement("script");
  if (script.src) {
    executable.src = resolveUrl(script.getAttribute("src"), sourceUrl);
  } else {
...
async function materializeRuntime(root, engineHost, surface) {
  ...
  runtimePromise = (async () => {
    ...
    const importedRuntime = document.importNode(functionalModule, true);
    importedRuntime.dataset.material3QuotesEngine = "true";
    engineHost.replaceChildren(importedRuntime);

    const input = importedRuntime.querySelector("#fq-solution-online-pdf-105dr");
    const sourceStatus = importedRuntime.querySelector(".fq-file-status-105dr");
    const projection = surface.querySelector("[data-material3-quotes-projection]");
    const selectButton = surface.querySelector("[data-material3-select-quote]");
    const selectedFile = surface.querySelector("[data-material3-selected-file]");
    const visibleStatus = surface.querySelector("[data-material3-quotes-status]");

    if (!input || !projection || !selectButton || !selectedFile || !visibleStatus) {
    ...
    const syncIntakeState = state => {
      const normalized = String(state || "empty").trim().toLowerCase();
      root.dataset.intakeState = normalized;
      syncStatus();
      if (["candidate_ready", "calculating", "ready"].includes(normalized)) {
        queueMicrotask(() => void refreshProjection());
      }
    };

    syncIntakeState(importedRuntime.dataset.forgeIntakeState);
    importedRuntime.addEventListener("forge:quote-intake-state-change", event => {
    ...
    input.addEventListener("change", () => {
      ...
      selectedFile.textContent = file?.name || "Sin archivo seleccionado";
    ...
    const observer = new MutationObserver(() => {
      ...
      observer.refreshTimer = window.setTimeout(() => void refreshProjection(), 80);
    });
    ...
    for (const script of documentSource.querySelectorAll("script")) {
      await appendRuntimeScript(script, sourceUrl);
    }
    ...
    async mount() {
      ...
      if (!mounted) {
        ...
        root.innerHTML = `
          ...
          <section class="quotes-module__upload" aria-labelledby="material3-quotes-upload-title">
            <p class="quotes-module__kicker">CARGA LOCAL</p>
            <h2 id="material3-quotes-upload-title">Carga tu cotización</h2>
            <p>Selecciona el PDF de Solución Online. Se procesa localmente en tu navegador.</p>
            <div class="quotes-module__upload-controls">
              <button type="button" data-material3-select-quote>Seleccionar PDF</button>
              <span data-material3-selected-file>Sin archivo seleccionado</span>
            </div>
            <p class="quotes-module__status" data-material3-quotes-status role="status">
          ...
          <div class="quotes-module__engine" data-forge-quotes-engine aria-hidden="true"></div>
        `;
