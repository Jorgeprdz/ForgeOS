...
export function createActivityRuntimeAdapter({ client, user, windowRef = window } = {}) {
  ...
  let ledger = null;
  let reporting = null;
  let calendarAuthorityState = "UNKNOWN";
  ...
  function captureDefinitions() {
    ...
    if (!requiredEventTypes.every((eventType) => canonical.EVENT_TYPES.includes(eventType))) {
      throw new Error("ACTIVITY_CAPTURE_TAXONOMY_INCOMPLETE");
    }
    return freeze([
      {
        captureType: "REFERRAL_RECEIVED",
        eventType: "REFERRAL_RECEIVED",
        subjectType: "ACTIVITY",
        fields: [
          { name: "activity_reference", type: "text" },
          { name: "referral_reference", type: "text" },
        ],
      },
      {
        captureType: "CALL_COMPLETED",
        eventType: "CALL_COMPLETED",
        subjectType: "ACTIVITY",
        fields: [
          { name: "activity_reference", type: "text" },
          { name: "contact_reference", type: "text" },
        ],
      },
      {
        captureType: "APPOINTMENT_SCHEDULED",
        eventType: "APPOINTMENT_SCHEDULED",
        subjectType: "APPOINTMENT",
        fields: [
          { name: "appointment_reference", type: "text" },
          { name: "starts_at", type: "datetime-local" },
          { name: "ends_at", type: "datetime-local" },
        ],
        discriminator: { appointment_purpose: "INITIAL" },
      },
      {
        captureType: "APPOINTMENT_HELD",
        eventType: "APPOINTMENT_HELD",
        subjectType: "APPOINTMENT",
        fields: [
          { name: "appointment_reference", type: "text" },
          { name: "outcome_confirmed_at", type: "datetime-local" },
        ],
        discriminator: { appointment_purpose: "INITIAL" },
      },
      {
        captureType: "CLOSING_APPOINTMENT_HELD",
        eventType: "APPOINTMENT_HELD",
        subjectType: "APPOINTMENT",
        fields: [
          { name: "appointment_reference", type: "text" },
        ...
        discriminator: { appointment_purpose: "CLOSING" },
      },
      {
        captureType: "ADVISOR_REFERRAL_RECEIVED",
        eventType: "ADVISOR_REFERRAL_RECEIVED",
        subjectType: "ACTIVITY",
        fields: [
          { name: "activity_reference", type: "text" },
          { name: "referred_advisor_reference", type: "text" },
        ],
      },
    ]);
  }
  ...
  async function openLedger() {
    ...
    if (!ledger) {
      ledger = ledgerApi.create({
        client,
        tenant_id: tenantId,
        device_id: deviceId(windowRef.localStorage),
      });
    }
    return ledger;
  }
  ...
  function createEventInput({
    ...
    if (!captureDefinitions().some((entry) =>
      entry.eventType === eventType && entry.subjectType === subjectType
    )) {
      throw new Error("ACTIVITY_EVENT_TYPE_NOT_WRITABLE");
    }
    ...
    return canonical.createCanonicalActivityEvent({
      ...
      payload,
      provenance: {
        source_system: "FORGE_AURA_ACTIVITY",
        source_record_id: subjectId,
        captured_via: "MANUAL_CONFIRMED",
        evidence_references: Object.values(payload).filter((value) => typeof value === "string"),
      },
  ...
  async function appendOne(input) {
    ...
    const result = await selected.appendCanonicalEvent({
      canonical_event: event,
      evidence_references: [],
    });
    const receipt = await selected.syncOnce();
    const pending = Boolean(receipt?.push_failed || receipt?.pending || receipt?.failed);
    return freeze({
      state: pending ? "PENDING_SYNC" : "CONFIRMED",
  ...
  async function loadCalendar({ from, to } = {}) {
    if (!isoDate(from) || !isoDate(to) || from > to) {
      return freeze({
        ...
        period: null,
        reason: "OFFICIAL_PERIOD_REQUIRED",
      });
    }
    const controller = new AbortController();
    aborters.add(controller);
    try {
      const repository = calendarRepositoryApi.createOperationalCalendarRepository({
        client,
        getSessionAdvisorId: async () => tenantId,
      });
      const records = await repository.readAuthority({
        from,
        to,
        signal: controller.signal,
      });
      calendarAuthorityState = "AVAILABLE";
      ...
      if (/OPCAL_.*READ_FAILED|42P01|relation .* does not exist/i.test(code)) {
        calendarAuthorityState = "UNAVAILABLE";
        return freeze({
          state: "CONFIGURATION_REQUIRED",
          period: { from, to },
      ...
      calendarAuthorityState = "ERROR";
      return freeze({
        state: "ERROR",
        period: { from, to },
        reason: code || "CALENDAR_READ_FAILED",
      });
    } finally {
  ...
  async function loadReporting(calendar, request) {
    ...
    if (!reporting) {
      reporting = await createProductiveActivityReportingBridge({
        ...
        ledgerRuntimeApi: ledgerApi,
        deviceId: deviceId(windowRef.localStorage),
    ...
    try {
      ...
      return freeze({
        state: "DISCONNECTED",
        reason: error?.code || error?.message || "REPORTING_UNAVAILABLE",
      });
    }
  }
  ...
  function combinations(remaining) {
    return Number.isInteger(remaining) && remaining > 0
      ? findPointCombinations(remaining)
      : [];
  }
  ...
  return freeze({
    ...
    diagnostics: () => freeze({
      ...
      directSupabaseActivityWrite: false,
      calendarAuthorityState,
      productionMigrationExecuted: calendarAuthorityState === "AVAILABLE",
    }),
  });
}
