...
function renderBoot(message, state = "AUTH_LOADING") {
  ...
  root.innerHTML = `<section class="aura-login" data-aura-auth-state="${state}"><div class="aura-loading"><div aria-hidden="true"></div><h1>${message}</h1><p>Forge mantiene los datos protegidos mientras verifica la sesión.</p></div></section>`;
}
...
function showLogin(message = "") {
  ...
  renderAuraLogin({
    root,
    auth,
    onAuthenticated: () => router.navigate("pipeline", { replace: true }),
  });
  if (message) {
...
async function boot() {
  ...
  auth.subscribe(snapshot => {
    if (snapshot.event === "SIGNED_OUT" || snapshot.event === "SESSION_EXPIRED") {
      router.navigate("login", { replace: true });
      showLogin(snapshot.event === "SESSION_EXPIRED" ? "Tu sesión expiró. Inicia sesión nuevamente." : "");
    }
  });
  try {
    ...
    showLogin(auth.humanAuthError(error));
  }
}
