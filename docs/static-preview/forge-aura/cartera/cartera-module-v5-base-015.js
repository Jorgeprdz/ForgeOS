import { createCarteraModule as createBaseCarteraModule } from './cartera-module.js?base=cartera-pdf-ingress-legacy-refresh';
import { createCarteraAdapter as createSemanticCarteraAdapter } from './cartera-adapter-pages-v9.js?v=cartera-pdf-ingress-legacy-refresh';
import {
...
function droppedFileCandidates(dataTransfer) {
  const direct = Array.from(dataTransfer?.files || []);
  const fromItems = Array.from(dataTransfer?.items || [])
    .filter(item => item?.kind === 'file')
    .map(item => {
      try { return item.getAsFile?.() || null; } catch { return null; }
    })
    .filter(Boolean);
  return [...direct, ...fromItems];
}

function normalizeDroppedPdf(file, windowRef) {
  if (!file || Number(file.size) < 1) return null;
  const name = String(file.name || '');
  const type = String(file.type || '').toLowerCase();
  const hasPdfName = /\.pdf$/i.test(name);
  const hasPdfMime = type === 'application/pdf';
  if (!hasPdfName && !hasPdfMime) return null;
  if (hasPdfName && hasPdfMime) return file;
  const FileCtor = windowRef.File || File;
  return new FileCtor([file], hasPdfName ? name : 'documento.pdf', {
    type: 'application/pdf',
    lastModified: Number(file.lastModified) || Date.now(),
  });
}

function selectDroppedPdf(dataTransfer, windowRef) {
  const candidates = droppedFileCandidates(dataTransfer);
  const preferred = candidates.find(file => /\.pdf$/i.test(String(file?.name || '')) && Number(file?.size) > 0)
    || candidates.find(file => String(file?.type || '').toLowerCase() === 'application/pdf' && Number(file?.size) > 0)
    || null;
  return normalizeDroppedPdf(preferred, windowRef);
}

function showDropError(drop) {
  drop.querySelector('[data-pdf-drop-error]')?.remove();
  drop.insertAdjacentHTML('beforeend', '<p data-pdf-drop-error class="cartera-error" role="alert">No pudimos leer el archivo soltado. Usa un PDF válido o selecciónalo desde el dispositivo.</p>');
}
...
export function createCarteraModule(options = {}) {
  ...
  let dropCaptureInstalled = false;
  ...
  function onDropCapture(event) {
    const drop = event.target?.closest?.('[data-pdf-drop]');
    if (!drop) return;
    event.preventDefault();
    ...
    const file = selectDroppedPdf(event.dataTransfer, windowRef);
    const input = drop.closest('.cartera-dialog')?.querySelector('[data-pdf-input]');
    if (!file || !input) {
      showDropError(drop);
      return;
    }
    ...
    try {
      const TransferCtor = windowRef.DataTransfer || DataTransfer;
      const transfer = new TransferCtor();
      transfer.items.add(file);
      input.files = transfer.files;
      input.dispatchEvent(new windowRef.Event('change', { bubbles: true }));
      drop.dataset.ingress = 'selector-parity';
    } catch {
      showDropError(drop);
    }
  }
  ...
  function startObserver() {
    ...
    if (!dropCaptureInstalled) {
      doc.addEventListener('drop', onDropCapture, true);
      dropCaptureInstalled = true;
    }
  }
  ...
  function stopObserver() {
    observer?.disconnect();
    observer = null;
    if (dropCaptureInstalled) doc.removeEventListener('drop', onDropCapture, true);
    dropCaptureInstalled = false;
    peoplePromise = null;
    lastReview = null;
    activeAdapter = null;
  }
