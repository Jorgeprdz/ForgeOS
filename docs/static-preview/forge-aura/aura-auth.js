import { oauthRedirectUrl } from "./aura-router.js";
...
async function loadSupabase() {
  ...
  supabaseLibraryPromise = new Promise((resolve, reject) => {
    ...
    script.onload = () => globalThis.supabase?.createClient ? resolve(globalThis.supabase) : reject(new Error("AUTH_CLIENT_INVALID"));
...
async function getClient() {
  ...
  clientPromise = (async () => {
    ...
    if (config.state !== "READY") {
      throw Object.assign(new Error(config.DEMO_MODE ? "PRODUCTIVE_AUTH_DISABLED_IN_DEMO" : "PRODUCTIVE_AUTH_CONFIG_MISSING"), { code: "CONFIG_BLOCKED" });
    }
...
export function createAuraAuth({ windowRef = window } = {}) {
  ...
  async function signInWithGoogle() {
    ...
    const { data, error } = await client.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: oauthRedirectUrl(windowRef.location.href), skipBrowserRedirect: false },
    });
    if (error) throw error;
    return data;
  }
...
export function renderAuraLogin({ root, auth, onAuthenticated } = {}) {
  ...
  form.addEventListener("submit", async event => {
    ...
    submit.disabled = true; submit.setAttribute("aria-busy", "true"); submit.textContent = "Entrando…";
    ...
    } catch (error) { setError(auth.humanAuthError(error)); }
    finally { submit.disabled = false; submit.removeAttribute("aria-busy"); submit.textContent = "Iniciar sesión"; }
  });
  root.querySelector("[data-aura-google]").addEventListener("click", async event => {
    const button = event.currentTarget; setError(""); button.disabled = true; button.setAttribute("aria-busy", "true");
    try { await auth.signInWithGoogle(); }
    catch (error) { setError(auth.humanAuthError(error)); button.disabled = false; button.removeAttribute("aria-busy"); }
  });
}
