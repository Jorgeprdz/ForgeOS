...
export const phoneOf = record => String(
  record?.phone ||
  record?.prospect?.phone ||
  record?.prospect?.whatsapp ||
  "",
).replace(/[^\d+]/g, "");

export const stageLabel = value =>
  STAGES.find(stage => stage.value === value)?.label || value || "Sin etapa";

export function formatDate(value, fallback = "Sin registro") {
  const time = Date.parse(value || "");
  return Number.isFinite(time)
    ? new Intl.DateTimeFormat("es-MX", { dateStyle: "medium", timeStyle: "short" }).format(new Date(time))
    : fallback;
}
...
export function matches(record, filters) {
  ...
  const haystack = normalize([
    ...
    record.latestActivity?.label,
    record.nextCommitment?.type,
  ].filter(Boolean).join(" "));
...
export function deriveCard(prospect, timeline = []) {
  const latest = [...timeline]
    .sort((left, right) => String(
      ...
      left.occurredAt || left.recordedAt || "",
    )))[0] || null;
  ...
  return Object.freeze({
    ...
    sourceSummary: [
      ...
      prospect.referrerRelationship,
    ].filter(Boolean).join(" · ") || "Fuente no disponible",
    productInterest: Array.isArray(prospect.productsOfInterest)
      ? prospect.productsOfInterest.join(", ")
      : prospect.productsOfInterest || "",
    phone: prospect.phone || prospect.whatsapp || null,
    latestActivity: latest
      ? {
          label: latest.eventType?.replaceAll("_", " ").toLowerCase(),
          occurredAt: latest.occurredAt || latest.recordedAt,
        }
      : null,
...
export function viewPreference(storage) {
  try {
    return storage?.getItem(VIEW_STORAGE_KEY) === "list" ? "list" : "cards";
  } catch {
    return "cards";
  }
}
...
export function saveViewPreference(storage, view) {
  try {
    storage?.setItem(VIEW_STORAGE_KEY, view === "list" ? "list" : "cards");
  } catch {
    // View preference is optional and never contains private prospect data.
  }
}
