export function parseSolucionlineRetirementQuote({ text = "" }) {
  ...
  const getNumber = (value) => Number(String(value || "").replace(/,/g, ""));
  ...
  const scenarioEvidence = scenarioMatch
    ? {
        base: { lumpSum: getNumber(scenarioMatch[2]), monthlyIncome: getNumber(scenarioMatch[3]) },
        favorable: { lumpSum: getNumber(scenarioMatch[4]), monthlyIncome: getNumber(scenarioMatch[5]) },
        unfavorable: { lumpSum: getNumber(scenarioMatch[6]), monthlyIncome: getNumber(scenarioMatch[7]) }
      }
    : {
        base: { lumpSum: null, monthlyIncome: null },
        favorable: { lumpSum: null, monthlyIncome: null },
        unfavorable: { lumpSum: null, monthlyIncome: null }
      };
  ...
  return {
    ...
    interestRate: interestRateMatch ? getNumber(interestRateMatch[1]) : null,
    evidence: {
