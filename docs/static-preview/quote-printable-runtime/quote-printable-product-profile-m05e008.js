...
function contractedProtectionRows(nativeResult, currentUdi) {
  ...
  const definitions = [
    { id: "pcf", label: "Protección por cáncer femenino", patterns: ["pcf", "cancer femenino"], kind: "amount" },
    { id: "bait", label: "Invalidez total y permanente", patterns: ["bait", "invalidez total"], kind: "amount" },
    { id: "bit", label: "Exención de pago de primas", patterns: ["bit", "exencion", "exención"], kind: "status" },
    { id: "bam", label: "Asistencia médica", patterns: ["bam", "asistencia medica", "asistencia médica"], kind: "status" },
    { id: "av", label: "Apoyo en vida", patterns: ["av ui", "apoyo en vida"], kind: "status" },
    { id: "bma", label: "Muerte accidental", patterns: ["bma", "muerte accidental"], kind: "amount" },
  ];
...
function buildVidaMujerCommercialSummary(reviewSnapshot) {
  ...
  const nativeResult = firstObject(calculation.nativeResult, acceptedQuote.nativeResult) || {};
  const rateMetadata = currentRateMetadata(reviewSnapshot, calculation, acceptedQuote, nativeResult);
  ...
  const mainCoverage = findCoverage(coverages, ["vida mujer", "fallecimiento", "basico", "básico"]);
  ...
  const sumAssuredMxn = firstPositive(
    ...
    sumAssuredUdi !== null && currentUdi !== null ? sumAssuredUdi * currentUdi : null,
  );
  ...
  const annualContributionMxn = annualContributionUdi !== null && currentUdi !== null
  ...
  const projectionRows = projectionCandidates(calculation, acceptedQuote, nativeResult);
  const endowments = sumAssuredUdi === null
    ? []
    : ENDOWMENT_SCHEDULE.map(({ policyYear, percentage }) => {
        const projectedUdiValue = projectedRateForYear(projectionRows, policyYear);
        const benefitUdi = sumAssuredUdi * percentage;
        return {
          policyYear,
          percentage,
          benefitUdi,
          benefitMxn: projectedUdiValue !== null ? benefitUdi * projectedUdiValue : null,
          projectedUdiValue,
          projectionStatus: projectedUdiValue === null
  ...
  const survivalTotalUdi = endowments.length
    ...
    : null;
  const survivalTotalMxn = endowments.length && endowments.every((item) => item.benefitMxn !== null)
  ...
  const pcfDiseases = pcf?.udi
    ? PCF_DISEASES.map((disease) => ({
        ...
        benefitMxn: currentUdi !== null ? pcf.udi * disease.percentage * currentUdi : null,
      }))
    : [];
  ...
  return deepFreeze({
    ...
    paymentYears,
    sumAssured: { udi: sumAssuredUdi, mxn: sumAssuredMxn },
    annualContribution: {
    ...
    protections,
    endowments,
    survivalTotal: { udi: survivalTotalUdi, mxn: survivalTotalMxn },
    pcfDiseases,
    evidence: {
      udiValue: currentUdi,
      udiDate: String(rateMetadata.source_date || rateMetadata.sourceDate || rateMetadata.date || "").trim() || null,
      udiSource: String(rateMetadata.source || "").trim() || null,
      seriesId: String(rateMetadata.series_id || rateMetadata.seriesId || rateMetadata.key || "").trim() || null,
...
function buildProductSpecificQuotePrintableReadModel({ readModel, reviewSnapshot } = {}) {
  const profiled = buildM05e007ProfiledReadModel({ readModel, reviewSnapshot });
  ...
  const warnings = [];
  if (commercialSummary.sumAssured.udi === null) warnings.push("La suma asegurada de Vida Mujer no está disponible.");
  if (commercialSummary.annualContribution.udi === null) warnings.push("La aportación anual de Vida Mujer no está disponible.");
  if (!commercialSummary.endowments.length) warnings.push("No fue posible construir el calendario de dotales.");

  return deepFreeze({
    ...clone(profiled),
    contractVersion: CONTRACT_VERSION,
    commercialSummary,
    review: { ...clone(profiled.review || {}), warnings },
    disclaimers: [
