...
const QPD06_ROUTE_CONTROLLER_VERSION =
  "M05E010_VIDA_MUJER_LANDSCAPE_ROUTE_CONTROLLER_V1";
...
function resolveOrientation(printableDocument, pdfPacket, fallback = "PORTRAIT") {
  return String(
    printableDocument?.pageOrientation ||
    pdfPacket?.pageOrientation ||
    fallback,
  ).toUpperCase();
}
...
function createQuotePrintableRouteController({
  ...
  if (typeof snapshotProvider !== "function") {
    fail(
      ...
      "Se requiere la fuente de cotización confirmada.",
    );
  }
  if (identityProvider !== null && typeof identityProvider !== "function") {
    fail(
      "IDENTITY_PROVIDER_INVALID",
      "La fuente de identidad durable no es válida.",
    );
  }
  ...
  async function ensureDurableIdentity() {
    ...
    const identity =
      identityFromLifecycleResult(result) || normalizeIdentity(result);
  ...
  function buildBundle({
    ...
    const reviewSnapshot = normalizePrintableReviewSnapshot(sourceSnapshot);
    const resolvedFormat = normalizePageFormat(requestedPageFormat);
    const readModel = buildQuotePrintableReadModel({
      reviewSnapshot,
      generatedAt,
    });
    const profiledReadModel = buildProductSpecificQuotePrintableReadModel({
      readModel,
      reviewSnapshot,
    });
    const printableDocument = buildQuotePrintableDocument({
      readModel: profiledReadModel,
      pageFormat: resolvedFormat,
      documentTitle:
        profiledReadModel.productProfile?.documentTitle || "Cotización",
    });
    const pdfPacket = buildQuotePrintablePdf({
      readModel: profiledReadModel,
      printableDocument,
      title: profiledReadModel.productProfile?.documentTitle || "Cotización",
      generatedAt,
    });
  ...
  async function download({
    ...
    if (userInitiated !== true) {
      fail(
        ...
        "La descarga requiere una acción humana explícita.",
      );
    }
  ...
  function reopen(printableVersionReference) {
    ...
    if (!record) {
      fail(
        "PRINTABLE_VERSION_NOT_FOUND",
        "La versión imprimible ya no está disponible.",
      );
    }
    const reopened = reopenQuotePrintableVersion({ record });
    const pageOrientation = resolveOrientation(
      reopened.printableDocument,
      reopened.pdfPacket,
  ...
  function state() {
    ...
    return deepFreeze({
      ...
      pageFormat,
      pageOrientation: activeBundle?.pageOrientation || "PORTRAIT",
