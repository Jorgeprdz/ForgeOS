...
const VERSION = '20260731000241';
const NAME = 'cartera020c_conflict_persistence_receipt_hardening';
const PATH = 'supabase/migrations/20260731000241_cartera020c_conflict_persistence_receipt_hardening.sql';
const ARTIFACT_DIR = 'artifacts/cartera-020c-remote-acceptance';
const LOG_PATH = `${ARTIFACT_DIR}/conflict-persistence-hardening.log`;
...
function retryDelay(attempt) { return 750 * (2 ** (attempt - 1)); }
...
async function readMigration() {
  const rows = await query(
    ...
    'READ_CONFLICT_PERSISTENCE_MIGRATION',
  );
  return rows[0] ?? null;
}
function historyInsert(raw, columns) {
  ...
  if (columns.has('name')) { names.push('name'); values.push(sqlLiteral(NAME)); }
...
async function main() {
  ...
  if (existing) {
    const stored = Array.isArray(existing.statements)
      ...
      : typeof existing.statements === 'string' ? normalize(existing.statements) : null;
    if (stored) assert.equal(sha256(stored), localHash, 'CARTERA020C_CONFLICT_PERSISTENCE_REMOTE_CONTENT_MISMATCH');
    log('MIGRATION_20260731000241=ALREADY_APPLIED_AND_MATCHED');
  } else {
    const columns = await migrationColumns();
    await query(
      `begin;\n${stripOuterTransaction(raw)}\n${historyInsert(raw, columns)}\ncommit;`,
      'APPLY_CARTERA020C_CONFLICT_PERSISTENCE_HARDENING',
    );
    assert.ok(await readMigration(), 'CARTERA020C_CONFLICT_PERSISTENCE_HISTORY_NOT_RECORDED');
    log('MIGRATION_20260731000241=APPLIED');
  }
  ...
  const rows = await query(
    ...
    'VERIFY_CARTERA020C_CONFLICT_PERSISTENCE_HARDENING',
  );
  const definition = String(rows[0]?.definition ?? '');
  assert.match(definition, /returning \* into persisted_conflict/i);
  assert.match(definition, /CARTERA020C_CONFLICT_PERSISTENCE_FAILED/);
  assert.match(definition, /CARTERA020C_CONFLICT_PERSISTENCE_MISMATCH/);
  assert.match(definition, /pg_advisory_xact_lock/);
  assert.doesNotMatch(definition, /on conflict/i);
  log('CARTERA020C_CONFLICT_PERSISTENCE_RECEIPT_HARDENING=PASS');
}

try {
  await main();
} catch (error) {
  log(`CARTERA020C_CONFLICT_PERSISTENCE_RECEIPT_HARDENING=FAIL:${String(error?.message ?? error)}`);
  process.exitCode = 1;
} finally {
  mkdirSync(ARTIFACT_DIR, { recursive: true });
