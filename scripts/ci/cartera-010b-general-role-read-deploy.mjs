...
const VERSION = "20260731000215";
const NAME = "cartera010b_general_policy_role_read_authority";
const PATH =
...
const LOG_PATH = `${ARTIFACT_DIR}/general-role-read-authority.log`;
...
async function readMigration() {
  const rows = await query(
    `select version, name, statements
      ...
      limit 1`,
    "READ_GENERAL_ROLE_READ_MIGRATION",
  );
  return rows[0] ?? null;
}
...
async function main() {
  ...
  if (existing) {
    ...
    if (stored) {
      assert.equal(
        sha256(stored),
        localHash,
        "CARTERA010B_GENERAL_ROLE_READ_REMOTE_CONTENT_MISMATCH",
      );
    }
    log("MIGRATION_20260731000215=ALREADY_APPLIED_AND_MATCHED");
  } else {
    const columns = await migrationColumns();
    const body = stripOuterTransaction(raw);
    await query(
      `begin;\n${body}\n${historyInsert(raw, columns)}\ncommit;`,
      "APPLY_GENERAL_ROLE_READ_AUTHORITY",
    );
    assert.ok(await readMigration(), "GENERAL_ROLE_READ_HISTORY_NOT_RECORDED");
    log("MIGRATION_20260731000215=APPLIED");
  }
  ...
  const verification = await query(
    `select
       ...
       pg_get_functiondef(
         'public.forge_cartera010b_list_general_policy_roles(text)'::regprocedure
       ) like '%r.role_type <> ''BENEFICIARY''%' as beneficiary_excluded,
       pg_get_functiondef(
         'public.forge_cartera010b_list_general_policy_roles(text)'::regprocedure
       ) like '%r.advisor_id = actor_id%' as owner_scoped
     from pg_proc p
     where p.oid =
       'public.forge_cartera010b_list_general_policy_roles(text)'::regprocedure`,
    "VERIFY_GENERAL_ROLE_READ_AUTHORITY",
  );
  assert.equal(verification[0]?.security_definer, true, "GENERAL_ROLE_READ_NOT_SECURITY_DEFINER");
  assert.equal(verification[0]?.authenticated_execute, true, "GENERAL_ROLE_READ_AUTHENTICATED_EXECUTE_MISSING");
  assert.equal(verification[0]?.anon_denied, true, "GENERAL_ROLE_READ_ANON_EXECUTE_NOT_REVOKED");
  assert.equal(verification[0]?.beneficiary_excluded, true, "GENERAL_ROLE_READ_BENEFICIARY_FILTER_MISSING");
  assert.equal(verification[0]?.owner_scoped, true, "GENERAL_ROLE_READ_OWNER_SCOPE_MISSING");
  log("CARTERA010B_GENERAL_POLICY_ROLE_READ_AUTHORITY=PASS");
}

try {
  await main();
} catch (error) {
  log(`CARTERA010B_GENERAL_POLICY_ROLE_READ_AUTHORITY=FAIL:${String(error?.message ?? error)}`);
  process.exitCode = 1;
} finally {
  mkdirSync(ARTIFACT_DIR, { recursive: true });
