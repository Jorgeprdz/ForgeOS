...
const SOURCE_HEAD = '655a0595515a4b3ed2ef1eb8d91f080939049f00';
const ACCEPTANCE_BRANCH = 'feature/cartera-040abcd-relationship-memory-product';
const AUTHORIZATION = 'YES:CARTERA_040ABCD_REMOTE_MUTATION';
const ACCEPTANCE_SQL = 'scripts/ci/cartera-040abcd-remote-acceptance.sql';
const ARTIFACT_DIR = 'artifacts/cartera-040abcd-remote-acceptance';
const REPORT_PATH = join(ARTIFACT_DIR, 'report.json');
const LOG_PATH = join(ARTIFACT_DIR, 'acceptance.log');

const MIGRATIONS = Object.freeze([
  ['20260801000270', 'cartera040_relationship_memory_authority'],
  ['20260801000271', 'cartera040_relationship_brief_read'],
...
const report = {
  phase: 'CARTERA_040ABCD_REMOTE_ACCEPTANCE',
  projectRef: PROJECT_REF,
  sourceHead: SOURCE_HEAD,
  acceptanceHead: process.env.CARTERA_040ABCD_ACCEPTANCE_HEAD ?? null,
  migrations: [],
...
assert.equal(process.env.CARTERA_040ABCD_REMOTE_MUTATION_AUTHORIZED, AUTHORIZATION);
assert.equal(process.env.SUPABASE_PROJECT_REF, PROJECT_REF);
assert.equal(process.env.CARTERA_040ABCD_EXPECTED_SOURCE_HEAD, SOURCE_HEAD);
assert.equal(process.env.CARTERA_040ABCD_ACCEPTANCE_BRANCH, ACCEPTANCE_BRANCH);
assert.ok(process.env.CARTERA_040ABCD_ACCEPTANCE_HEAD);
...
async function verifyCatalog() {
  const rows = await query(`
    select
      to_regclass('public.cartera040_relationship_memory_entries') is not null as memory_table,
      to_regclass('public.cartera040_relationship_memory_conflicts') is not null as conflict_table,
      to_regclass('public.cartera040_command_receipts') is not null as receipt_table,
      to_regprocedure('public.forge_cartera040_record_relationship_memory(jsonb)') is not null as record_rpc,
      to_regprocedure('public.forge_cartera040_list_relationship_brief(jsonb)') is not null as brief_rpc
  `, 'VERIFY_CATALOG');
  for (const [key, value] of Object.entries(rows[0] ?? {})) {
    assert.equal(value, true, `CATALOG_${key.toUpperCase()}_FAILED`);
  }
  report.catalogVerification = 'PASS';
  log('CARTERA_040ABCD_CATALOG_VERIFICATION=PASS');
}
...
async function verifyDigestCompatibility() {
  ...
  const rows = await query(
    `select public.forge_cartera030b_digest(${literal(JSON.stringify(payload))}::jsonb) as digest`,
...
async function runTransactionalAcceptance() {
  ...
  assert.match(sql, /rollback;\s*select\s+'PASS CARTERA040_TRANSACTIONAL_ACCEPTANCE'/is);
  const rows = await query(sql, 'CARTERA040_TRANSACTIONAL_ACCEPTANCE');
  assert.ok(rows.some(row => row.acceptance === 'PASS CARTERA040_TRANSACTIONAL_ACCEPTANCE'));
  report.transactionalAcceptance = 'PASS';
  report.fixturesRolledBack = true;
  log('CARTERA_040ABCD_TRANSACTIONAL_ACCEPTANCE=PASS');
  log('DURABLE_RELATIONSHIP_MEMORY=PASS');
  log('UNIFIED_RELATIONSHIP_HISTORY=PASS');
  log('NETWORK_CONTEXT_PROJECTION=PASS');
  log('SENSITIVE_CONTEXT_CONSENT_GATE=PASS');
  log('CHANGED_INPUT_CONFLICT=PASS');
  log('SANITIZED_PRE_CONTACT_BRIEF=PASS');
...
async function verifyResidualFixtures() {
  const rows = await query(`
    select (
      (select count(*) from auth.users where email like 'cartera040-acceptance-%@forge.invalid')
      + (select count(*) from public.commercial_people where person_reference like 'CARTERA040_ACCEPTANCE:%')
      + (select count(*) from public.cartera040_relationship_memory_entries where person_reference like 'CARTERA040_ACCEPTANCE:%')
      + (select count(*) from public.commercial_accounts where account_reference like 'CARTERA040_ACCEPTANCE:%')
      + (select count(*) from public.canonical_policies where policy_reference like 'CARTERA040_ACCEPTANCE:%')
      + (select count(*) from public.cartera030c_confirmed_payment_events where payment_event_reference like 'CARTERA040_ACCEPTANCE:%')
      + (select count(*) from public.cartera040_relationship_memory_conflicts where claims ->> 'idempotencyKey' like 'CARTERA040_ACCEPTANCE:%')
...
try {
  log(`CARTERA_040ABCD_ACCEPTANCE_HEAD=${process.env.CARTERA_040ABCD_ACCEPTANCE_HEAD}`);
  const columns = await migrationColumns();
  for (const migration of MIGRATIONS) await applyMigration(migration, columns);
  log('CARTERA_040ABCD_REMOTE_DEPLOYMENT=PASS');
  await verifyCatalog();
  await verifyDigestCompatibility();
  await runTransactionalAcceptance();
  await verifyResidualFixtures();
  log('CARTERA_040ABCD_REMOTE_ACCEPTANCE=PASS');
  log('CARTERA_040A_COMPLETE=YES');
  log('CARTERA_040B_COMPLETE=YES');
  log('CARTERA_040C_COMPLETE=YES');
  log('CARTERA_040D_COMPLETE=YES');
  log('CARTERA_040_COMPLETE=YES');
  persist();
} catch (error) {
  report.error = sanitize(error?.stack ?? error?.message ?? error);
  log(`CARTERA_040ABCD_REMOTE_ACCEPTANCE=FAIL:${report.error}`);
  persist();
  throw error;
}
