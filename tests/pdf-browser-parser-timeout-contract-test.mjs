...
assert.match(parser, /import\(candidate\.module\),\s*12000,\s*"La carga de PDF\.js"/s);
assert.match(parser, /pdfjsPromise107z15p2R11E,\s*15000,\s*"La inicialización de PDF\.js"/s);
assert.match(parser, /file\.arrayBuffer\(\),\s*12000,\s*"La lectura del archivo PDF"/s);
assert.match(parser, /documentTask\.promise,\s*30000,\s*"La apertura del documento PDF"/s);
assert.match(parser, /pdf\.getPage\(pageNumber\),\s*12000,\s*`La carga de la página \$\{pageNumber\}`/s);
assert.match(parser, /page\.getTextContent\(\),\s*12000,\s*`La extracción de texto de la página \$\{pageNumber\}`/s);
assert.match(parser, /parsePdfFileToAcceptedQuotePacket\(file,[\s\S]*60000,[\s\S]*"El procesamiento completo del PDF"/);
...
console.log("PASS R16J1C1 PDF extraction timeout contract", {
