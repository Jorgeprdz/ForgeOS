...
const locale = JSON.parse(await readFile(
  new URL(
    "../docs/static-preview/forge-aura/activity/es-MX.json",
    import.meta.url,
  ),
  "utf8",
));
const { createActivityTipPresenter } = await import(
  "../docs/static-preview/forge-aura/activity/activity-tip-presenter.js"
);

const resolution = policyApi.resolvePolicySnapshot([policy], {
...
test("validates the one active Activity coaching policy snapshot", () => {
  ...
  assert.equal(
    resolution.policySnapshotId,
    "FORGE_ACTIVITY_COACHING_POLICY_V1@1.0.0",
  );
  assert.equal(
    policyApi.resolvePolicySnapshot([], {
      asOf: "2026-08-05T18:00:00.000Z",
    }).state,
    "UNAVAILABLE_OR_CONFLICTING",
  );
  assert.equal(
    policyApi.resolvePolicySnapshot([policy, policy], {
      asOf: "2026-08-05T18:00:00.000Z",
    }).state,
    "UNAVAILABLE_OR_CONFLICTING",
  );
...
test("generates only structured tips and limits them through the snapshot", () => {
  const output = intelligenceApi.generateActivityTips({
    ...
    pointCombinations: [{
      counts: { llamadas: 3, citas_agendadas: 1 },
      totalPoints: 6,
      excessPoints: 1,
      exact: false,
      reachesTarget: true,
...
test("does not generate tips without a valid snapshot", () => {
  const output = intelligenceApi.generateActivityTips({
    policyResolution: { state: "UNAVAILABLE_OR_CONFLICTING" },
  });
...
test("locks Aura route, live authorities, self-service calendar and honest partial points", async () => {
  ...
  const [
    ...
    readFile(new URL("activity/activity-calendar-controller.js", base), "utf8"),
    readFile(new URL("activity/activity-metrics-adapter.js", base), "utf8"),
    readFile(
      new URL(
    ...
    readFile(
      new URL("../scripts/build-activity-aura-pages-artifact.mjs", import.meta.url),
      "utf8",
    ),
    readFile(
      new URL(
  ...
  assert.match(html, /\.\.\/\.\.\/platform\/event-evidence\/activity-ledger-browser-runtime\.js/);
  assert.match(html, /\.\.\/\.\.\/platform\/productivity\/activity-conversion-read-model\.js/);
  ...
  assert.match(runtime, /syncOnce/);
  assert.match(runtime, /productionMigrationExecuted:\s*calendarAuthorityState === "AVAILABLE"/);
  assert.doesNotMatch(runtime, /productionMigrationExecuted:\s*false/);

  assert.match(module, /createActivityCalendarController/);
  assert.match(module, /createActivityMetricsAdapter/);
  assert.match(module, /describeActivityPointsAuthority/);
  assert.match(module, /data-calendar-config/);
  assert.match(module, /data-vacation/);
  assert.match(module, /confirmedMinimum/);
  assert.match(module, /activity-chart__line/);
  assert.match(module, /data-activity-slider/);
  assert.match(module, /data-activity-number/);
  assert.doesNotMatch(
    module,
    /dailyGoal|consecutiveEligibleDays|strictlyGreaterThanPoints|completedEligibleWeeks|maxVisibleTips/,
  );
  assert.doesNotMatch(module, /America\/Mexico_City/);

  assert.match(calendarController, /browserTimeZoneCandidate/);
  assert.match(calendarController, /browserTimezoneAutoSaved:\s*false/);
  assert.match(calendarController, /defaultWorkingWeekdays:\s*false/);
  assert.match(repository, /appendAdvisorProfile/);
  assert.match(repository, /appendTimeOff/);
  assert.match(repository, /command_digest/);

  assert.match(metricsAdapter, /APPLICATION_SUBMITTED/);
  assert.match(metricsAdapter, /paid_policy_confirmation_authority_unavailable/);
  assert.match(metricsAdapter, /policyPaidInferredFromIssuedPolicy:\s*false/);
  assert.doesNotMatch(metricsAdapter, /canonical_policies/);

  assert.match(reportingBridge, /\.\.\/\.\.\/advisor-os\/reporting/);
  assert.doesNotMatch(reportingBridge, /\.\.\/\.\.\/\.\.\/advisor-os/);
  assert.match(buildScript, /activity-authority-manifest\.json/);
  assert.match(buildScript, /businessRulesManuallyCopied:\s*false/);
  ...
  assert.match(styles, /activity-chart__target/);
});

test("resolves all critical Pages paths inside the ForgeOS project", () => {
  const htmlUrl = new URL(
    "https://jorgeprdz.github.io/ForgeOS/static-preview/forge-aura/auth-v4.html",
  );
  assert.equal(
    new URL("../../platform/event-evidence/canonical-activity-event-contract.js", htmlUrl).pathname,
    "/ForgeOS/platform/event-evidence/canonical-activity-event-contract.js",
  );

  const moduleUrl = new URL(
    "https://jorgeprdz.github.io/ForgeOS/static-preview/forge-aura/activity/activity-module.js",
  );
  assert.equal(
    new URL("../../../platform/productivity/activity-points-authority-adapter.mjs", moduleUrl).pathname,
    "/ForgeOS/platform/productivity/activity-points-authority-adapter.mjs",
  );

  const bridgeUrl = new URL(
    "https://jorgeprdz.github.io/ForgeOS/static-preview/forge-alive-material3/activity-ledger-reporting-bridge.mjs",
  );
  assert.equal(
    new URL("../../advisor-os/reporting/runtime/activity-reporting-runtime.mjs", bridgeUrl).pathname,
    "/ForgeOS/advisor-os/reporting/runtime/activity-reporting-runtime.mjs",
  );
});
