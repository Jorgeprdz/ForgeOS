...
assert.match(files.home, /canonicalHomeActions/);
assert.match(files.home, /replacedByProductiveSmartWidgets/);
assert.match(files.home, /function canonicalizeSmartWidgets/);
assert.match(files.home, /data\.canonicalRole|dataset\.canonicalRole/);
...
assert.match(files.home, /productive-smart-widget-why-toggle/);
assert.match(files.home, /¿Por qué\?/);
assert.match(files.home, /supportingIntelligence/);
assert.match(files.home, /intelligenceAbsorbed/);
assert.doesNotMatch(files.home, /createFipProductiveHomeBridge/);
assert.doesNotMatch(files.home, /home-fip-intelligence/);
assert.doesNotMatch(files.home, /\.plan-card \.mini-action/);
assert.doesNotMatch(files.home, /\.next-card \.primary-action/);
assert.doesNotMatch(files.home, /\.next-card \.save-action/);
...
console.log("HOME_CANONICAL_ACTION_CARDS=PASS");
console.log("HOME_STATIC_DEMO_ACTIONS_REMOVED=PASS");
console.log("HOME_INTELLIGENCE_ABSORPTION=PASS");
console.log("HOME_EXPLAINABLE_WHY_DISCLOSURE=PASS");
