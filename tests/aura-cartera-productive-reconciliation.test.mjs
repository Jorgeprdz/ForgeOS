...
test('document review wins semantic deduplication for the same policy', async () => { const { deriveAttention }=await import(`${coreUrl}?document-priority=${Date.now()}`); const items=deriveAttention({policies:[{policy_reference:'policy:1',policy_number:'SYN-1',completeness_state:'PARTIAL',conflict_state:'CLEAR'}],radar:{items:[{signalType:'INCOMPLETE_POLICY_DATA',eventDate:'2026-08-11',whyNow:'El documento necesita revisión.',smallestUsefulAction:'Revisar el documento.',policyReference:'policy:1'}]}}); assert.equal(items.length,1); assert.equal(items[0].title,'Documento pendiente de revisión'); assert.equal(items[0].action.label,'Abrir póliza'); });
test('relationship/productivity context remains advisory, useful and non-manipulative', () => { assert.match(adapterSource,/no score, probabilidad ni acción automática/); assert.match(moduleSource,/Información confirmada que te ayuda a dar continuidad/); assert.doesNotMatch(carteraSource,/auto(?:matic)?(?:Message|Opportunity|Decision)|manipulat/i); });
