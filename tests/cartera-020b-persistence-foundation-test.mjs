...
const migrationPath = 'supabase/migrations/20260731000220_cartera020b_persistent_evidence_worker.sql';

test('migration creates durable source, inbox, transition, attempt, candidate and packet authorities', async () => {
...
test('migration stores digest and opaque references but never raw document text', async () => {
  const sql = await read(migrationPath);
...
test('migration provides governed admission, claim and result commands', async () => {
  const sql = await read(migrationPath);
  for (const fn of [
...
test('idempotent replay and changed-input conflict are explicit and persistent', async () => {
  const sql = await read(migrationPath);
  assert.match(sql, /IDEMPOTENT_REPLAY|idempotentReplay/);
  assert.match(sql, /CHANGED_INPUT_REPLAY/g);
  assert.match(sql, /cartera020b_command_receipts/);
  assert.match(sql, /cartera020b_command_conflicts/);
  assert.doesNotMatch(sql, /raise exception 'CHANGED_INPUT_REPLAY'/);
});

test('worker supports leases, retries, expired-claim recovery and sequential isolation metadata', async () => {
  const sql = await read(migrationPath);
  for (const marker of [
    'lease_owner', 'lease_token', 'lease_expires_at', 'retry_count', 'next_retry_at',
    'state_version', 'FOR UPDATE SKIP LOCKED', 'RETRY_WAIT', 'LEASE_SECONDS_OUT_OF_BOUNDS',
  ]) assert.ok(sql.toUpperCase().includes(marker.toUpperCase()), `missing ${marker}`);
});

test('RLS is owner scoped and all direct mutations are revoked', async () => {
  const sql = await read(migrationPath);
...
test('020B migration cannot create Person, Policy, PolicyRole or external effects', async () => {
  const sql = await read(migrationPath);
  for (const forbidden of [
    ...
    'calendar',
    'message_send',
  ]) assert.equal(sql.toLowerCase().includes(forbidden.toLowerCase()), false, `forbidden ${forbidden}`);
  assert.match(sql, /'createsPolicy', false/);
});

test('candidate and packet rows are structurally non-truth and pending confirmation', async () => {
  const sql = await read(migrationPath);
