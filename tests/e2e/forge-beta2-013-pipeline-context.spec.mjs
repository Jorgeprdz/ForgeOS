...
test('015R decision context shows one concise conclusion for each governed state', async ({ page }) => {
  for (const scenario of [
    ...
    { mode: 'relationship', state: 'CONTEXT_INCOMPLETE_BUT_ACTIONABLE', file: 'incomplete-actionable', conclusion: 'Falta completar una parte del contexto', action: 'Confirmar si el evento sigue vigente' },
    { mode: 'empty', state: 'CONTEXT_INSUFFICIENT', file: 'insufficient', conclusion: 'Todavía no puedo recomendar un siguiente paso.', action: 'Completar información' },
  ]) {
    ...
    if (scenario.state === 'CONTEXT_INSUFFICIENT') {
      expect((visible.match(/Todavía no puedo recomendar/gi) || []).length).toBe(1);
      await expect(dialog.locator('[data-aura-governed-projections="EMPTY"]')).toHaveCount(0);
    }
