...
STATUS = Path(".forge/rep16e_reconcile_status.txt")
...
def main():
    ...
    quote_flags = '''      quoteResultsVisible: quoteReady,
      quoteReadyState:
    ...
    diagnostic = sub_once(
        diagnostic,
        r'^      quoteResultsVisible:.*?^      quoteProjectionBounded:',
        quote_flags,
        "quote capture flags",
    )
    ...
    diagnostic = replace_once(
      ...
      { state: "visible", timeout: 40_000 },
        ...
        "quote ready selector",
    )

    diagnostic = sub_once(
        diagnostic,
        r'^    result\.routes\.quotesEvidenceClosed = await capture\(.*?^    await page\.locator\("\[data-quote-technical-evidence\] summary"\)\.click\(\);\n',
...
''',
        "obsolete technical evidence accordion",
    )
    ...
    || "PRODUCT INTELLIGENCE";''',
        '''  eyebrow.textContent = snapshot.identity.family || "PRODUCT INTELLIGENCE";''',
        "human quote eyebrow",
    )
