...
async function processNote({ prospectId, advisorId = null, note }) {
  ...
  const alphaEvent = await repository.saveEvent(prospectId, note, runtimeResult.extracted_events);

  // 7. Persist forge_output.
  const forgeOutput = await repository.saveOutput(alphaEvent.id, runtimeResult);
