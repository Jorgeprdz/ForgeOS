...
-- Hardens the server context boundary discovered during schema verification.
-- REMOTE_APPLY=NOT_AUTHORIZED.

begin;

-- The first 011D context helper is not a browser API. Keep economic context behind the server orchestrator.
revoke all on function public.forge_advisor_compensation_handoff_context_011d(text)
  from public, anon, authenticated;

create or replace function public.forge_advisor_compensation_handoff_context_server_011d(
  p_actor_id uuid,
...
begin
  ...
  if requested_reference is null
    ...
    return jsonb_build_object('state','PAYMENT_NOT_FOUND');
  end if;

  select e.* into payment_row
  from public.cartera030c_confirmed_payment_events e
  where e.payment_event_reference = requested_reference
  limit 1;
  ...
  if payment_row.advisor_id <> p_actor_id then
    return jsonb_build_object(
  ...
  if payment_row.confirmation_state <> 'CONFIRMED' then
    return jsonb_build_object(
  ...
  left join lateral (
    ...
    where pr.advisor_id = e.advisor_id
      ...
      and pr.role_type in ('POLICY_OWNER','PAYOR','INSURED')
    order by
      case pr.role_type
...
revoke all on function public.forge_advisor_compensation_handoff_context_server_011d(uuid,text)
  from public, anon, authenticated;
grant execute on function public.forge_advisor_compensation_handoff_context_server_011d(uuid,text)
  to service_role;

comment on function public.forge_advisor_compensation_handoff_context_server_011d(uuid,text) is
  '011D service-only canonical Cartera context resolver. Explicit AUTH_REQUIRED/PAYMENT_NOT_FOUND/OWNER_MISMATCH/PAYMENT_NOT_CONFIRMED/ACCEPTED states; no calculation or mutation.';
