...
const optionalText = (value, max = 500) => {
  ...
  return text(String(value), "El texto opcional", max);
};
...
function normalizeSource(input = {}) {
  ...
  return freeze({
    id: text(input.id, "La fuente", 120),
    status,
...
function normalizeInsight(input = {}) {
  ...
  return freeze({
    id: text(input.id, "El insight", 160),
    kind,
    title: text(input.title, "El título", 180),
    summary: text(input.summary, "El resumen", 900),
...
function normalizeWidget(input = {}) {
  ...
  return freeze({
    id: text(input.id, "El widget", 160),
    surface,
    title: text(input.title, "El título", 180),
    state: text(input.state || "READY", "El estado", 80).toUpperCase(),
    insightIds: Array.isArray(input.insightIds) ? input.insightIds.map(id => text(id, "El insight", 160)) : [],
    deepLink: optionalText(input.deepLink, 500),
    readOnly: true,
...
export function createAlfredProductiveExperience(input = {}) {
  ...
  return freeze({
    ...
    orchestration: freeze({
      alfredRole: "ORCHESTRATOR",
      relationshipIntelligenceRole: "PERSON_CONTEXT",
      advisorIntelligenceRole: "ADVISOR_CONTEXT",
      mickRole: "EXECUTION_CONTEXT",
      nashRole: "COMMERCIAL_REASONING",
      opportunityRole: "OPERATION_CONTEXT",
      businessIntelligenceRole: "BUSINESS_CONTEXT",
    }),
    boundaries: freeze({
      factsSeparatedFromEstimates: true,
      estimatesSeparatedFromHypotheses: true,
      recommendationsSeparatedFromActions: true,
      unknownAsZero: false,
      ...
      logoutScrubRequired: true,
      lateResultRejectionRequired: true,
      mobileSafeZoneRequired: true,
    }),
  });
}
